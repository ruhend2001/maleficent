...
exports.default = {
   ...
   start: async (m, {
      ...
      const audio = await Scraper.ocean(link, 'mp4', 144).catch(async () => await Scraper.ocean(link, 'mp4', 360)).catch(async () => await Scraper.ocean(link, 'mp4', 480)).catch(async () => await Scraper.ocean(link, 'mp4', 720)).catch(async () => await Scraper.ocean(link, 'mp4', 1080)).catch(async () => await toBuffer((await (await fetch(`https://api.azbry.com/api/download/ytmp3?url=${link}`)).json()).result.download));
      const pretty = await Format.mp3Play(audio?.media || audio);    
      if (audio.media) m.react('✅')
      else if (audio && !audio.media) m.react('🔥')      
      return await conn.sendFile(m.chat, pretty, url.title, m);      
      /*
