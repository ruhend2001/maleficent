...
const { restoreGitlab } = require('../../lib/src/cloud/gitlab-db.js');
const { restoreSupabase } = require('../../lib/src/cloud/supabase-db.js');
const { restoreNeon } = require('../../lib/src/cloud/neon-db.js');
exports.default = {
   names: ['Tools'],
   tags: ['on', 'off'],
   ...
   start: async (m, {
      ...
      const off = async () => {
         ...
         await save.global('global.backup_neon = true', 'global.backup_neon = false');         
      };
      ...
      if (cmd_on.includes(command) && /autobackupall/g.test(text)) {
         ...
         if (!order.includes(pick)) return m.reply('main harus salah satu dari: mongo, github, gitlab, supabase, neon');
         const list = [pick, ...order.filter(x => x !== pick)];
         if (cmd_on.includes(command)) {            
            await m.reply(`Menyalakan auto backup semua database...`)                            
            await off();
            await Format.autoBackupAllDB(m, list, restoreMongo, restoreGithub, restoreGitlab, restoreSupabase, restoreNeon);            
      ...
      if (/autobackup/g.test(text) || text.split(" ")[1]) {
         ...
         if (!['mongo', 'github', 'gitlab', 'supabase', 'neon'].includes(pick)) throw 'hanya ada (mongo, github, gitlab, supabase, neon) saja sekarang' + backupall;
         ...
         } else if (cmd_on.includes(command) && pick === 'neon') {
            if (backup_neon) throw 'autobackup neon sudah di aktifkan atau di nyalakan sebelum nya untuk cek ketik .status';
            m.reply('menyalakan auto backup db ke cloud neon...')
            const data = await restoreNeon();
            if (!data.status) {
               await m.reply('Gagal Menyalakan autobackup neon')
               throw data
            } else if (data.status) {
               await save.global('global.backup_neon = false', 'global.backup_neon = true')
               return await m.reply('auto backup neon database berhasil di aktifkan\nrestarting...'), reset()
            }
         } else if (cmd_off.includes(command) && pick === 'neon') {
            if (!backup_neon) throw 'autobackup neon sudah di nonaktifkan atau dimatikan sebelumnya\nuntuk cek ketik .status';
            await m.reply('mematikan auto backup db ke cloud neon...')
            save.global('global.backup_neon = true', 'global.backup_neon = false');
            return m.reply('auto backup database neon berhasil di matikan')
         }
      }
   }
}