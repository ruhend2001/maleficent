...
const connect = async (m, {
   ...
   const Number = m.sender.replace(/[@+\s-]/g, '').replace('s.whatsapp.net', '').replace('lid', '');
   const num_1 = m.jid(Number + '@lid');   
   const num = num_1 == undefined ? Number + '@lid' : num_1.endsWith('@s.whatsapp.net') ? num_1 : Number + '@lid';
   if ((db.users[num]?.banned || db.users[Number + '@lid']?.banned) && !m.isBaileys) {
