...
const connect = async (m, {
   ...
   const isOwner = m.isLid ? (setting.ownerNumber.includes(mebot) || [...setting.ownerNumber, isMe].map(num => `${num}@s.whatsapp.net`).includes(m.sender)) : [...setting.ownerNumber, isMe].map(num => `${num}@s.whatsapp.net`).includes(m.sender)
   ...
   const groupAdmins = m.isLid ? await spam.getAdmins(m, conn, groupMetadata, participants) : await conn.getAdmins(m, groupMetadata, participants);
   ...
   if (Object.keys(settings).length < 1) global.db.settings = { menu_type: 2, prefix: 'multi', cover: setting.thumbnail, status: 0, readsw: true, antispam: true, block_pc: false, auto_bio: true, auto_down: false, auto_sticker: false, auto_clear_chat: false };
