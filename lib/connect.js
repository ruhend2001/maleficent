...
const connect = async (m, {
   ...
   const isOwner = [...setting.ownerNumber, isMe].map(num => `${num}@s.whatsapp.net`).includes(m.sender) || [...setting.ownerNumber, isMe].map(num => `${num}@lid`).includes(m.sender);
   const quoted = m.quoted ? m.quoted : m;
   const mime = m.quoted ? (m?.quoted?.mimetype || m?.quoted?.mtype) : m.mtype;
   const groupMetadata = m.isGroup ? await conn.cacheGroupMetadata(conn, m.chat) : {};
   const groupId = m.isGroup ? groupMetadata?.id : '';
   const groupName = m.isGroup ? groupMetadata?.subject : '';
   const groupDesc = m.isGroup ? groupMetadata?.desc : '';
   const participants = m.isGroup ? groupMetadata?.participants : '';
   const groupAdmins = m.isGroup ? groupMetadata.addressingMode === 'pn' ? participants?.filter(v => v.admin !== null).map(v => v.id) : participants?.filter(v => v.admin !== null).map(v => v.phoneNumber) : '';
   ...
   Format.Logger(m, { conn, time, chalk, pushname, groupName, body }), Connect(m, extra, register); 
