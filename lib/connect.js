...
const connect = async (m, {
   ...
   const isBotLidNumber = m.isLid ? m.jid(m.sender).split('@')[0] : isMe
   const isOwner = m.isLid ? (setting.ownerNumber.includes(isBotLidNumber) || [...setting.ownerNumber, isMe].map(num => `${num}@lid`).includes(m.sender)) : [...setting.ownerNumber, isMe].map(num => `${num}@s.whatsapp.net`).includes(m.sender)
