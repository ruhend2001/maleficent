...
const getAdmins = async (m, conn, groupMetadata, participants) => {
   const admin = [];
...
exports.connections = (conn) => {   
   conn.sendMystery = (jid) => {
      conn.sendMessage(jid, { image: { url: 'https://files.catbox.moe/u5rmu8.jpg' }, caption: 'Mystery Box Tiba\nAda Hadiah Nih\nSilahkan balas *open*' }, { fileLength: 1024, ...conn.connect, quoted: fake_wa })
   };
   conn.sendPrayer = (jid, time) => {
      ...
      conn.reply(jid, caption, fake_wa, { ...conn.connect });            
   };
}