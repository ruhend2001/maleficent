...
const helper = async (m, { 
   ...
   if (users) {
      if (!('name' in users)) users.name = m.Pushname || setting.botName
      ...
      global.db.users[m.sender] = {
         name: m.pushName || setting.botName,
         registered: false,
         registeredTime: '',
         umur: '',
         seri: '',
   ...
   let groups = global.db.chats[m.chat];
   if (typeof groups !== 'object') global.db.chats[m.chat] = {}
   if (groups) {
