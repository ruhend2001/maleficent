...
const helper = async (m, {
   ...
   const budy = typeof m.text == 'string' ? m.text : '', body = m.mtype === 'conversation' ? m.message.conversation : (m.mtype == 'imageMessage') ? m.message.imageMessage.caption : (m.mtype == 'videoMessage') ? m.message.videoMessage.caption : (m.mtype == 'extendedTextMessage') ? m.message.extendedTextMessage.text : (m.mtype == 'buttonsResponseMessage') ? m.message.buttonsResponseMessage.selectedButtonId : (m.mtype == 'listResponseMessage') ? m.message.listResponseMessage.singleSelectReply.selectedRowId : (m.mtype == 'templateButtonReplyMessage') ? m.message.templateButtonReplyMessage.selectedId : (m.mtype == 'interactiveResponseMessage') ? JSON.parse(m.msg.nativeFlowResponseMessage.paramsJson).id : (m.mtype == 'templateButtonReplyMessage') ? m.msg.selectedId : (m.mtype === 'messageContextInfo') ? (m.message.buttonsResponseMessage?.selectedButtonId || m.message.listResponseMessage?.singleSelectReply.selectedRowId) : '' || m.text ? m?.mtype === "reactionMessage" ? 'reaction ' + m?.msg?.text : m.text : m.text
   ...
   if (isPrefix.find(v => body.startsWith(v)) && db.settings.prefix === 'single') {
   ...
   if (budy.startsWith('>') || budy.startsWith('©')) {
      ...
      try {
         m.reply(`> evaling...`)
         await m.reply(format(await eval(`(async () => { ${budy.slice(2)} })()`)))
      } catch (e) {
   ...
   if (budy.startsWith('=>') || budy.startsWith('->') || budy.startsWith('~>')) {
      if (!isOwner || m.isBaileys) return
      try {
         await m.reply(format(await eval(`(async () => { return ${budy.slice(3)} })()`)));
      } catch (e) {
   ...
   if (budy.startsWith('$') || budy.startsWith('%') || budy.startsWith('&')) {
      if (!isOwner || m.isBaileys) return      
      m.reply(`> executing...`);
      exec(budy.slice(2), async (error, stdout) => {
