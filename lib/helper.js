...
const helper = async (m, {
   ...
   if (command && Spam.isFiltered(m.sender) && !m.isGroup && !m.isBaileys && !(prefix === undefined || prefix === '')) {
      return spam_private();
   } else if (command && Spam.isFiltered(m.sender) && m.isGroup && !m.isBaileys && !(prefix === undefined || prefix === '')) {
      return spam_group();
   };
   if (command && args.length < 1 && !m.isBaileys) Spam.addFilter(m.sender);
