...
import { Random } from './sock.js';
import webp from "node-webpmux";

async function imageToWebp(media) {
    const tmpFileOut = `./tmp/${Random()}.webp`
    const tmpFileIn = `./tmp/${Random()}.jpg`
    ...
    await new Promise((resolve, reject) => {
        ff(tmpFileIn)
            ...
            .addOutputOptions([
                ...
                "libwebp",
                "-vf",
                "scale='min(280,iw)':min'(280,ih)':force_original_aspect_ratio=decrease,fps=15, pad=280:280:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse"
            ])
            .toFormat("webp")
            .save(tmpFileOut)
    })
...
async function videoToWebp (media) {
    const tmpFileOut = `./tmp/${Random()}.webp`
    const tmpFileIn = `./tmp/${Random()}.mp4`
    fs.writeFileSync(tmpFileIn, media)
    await new Promise((resolve, reject) => {
        ff(tmpFileIn)
            .on("error", reject)
...
async function writeExifImg (media, metadata) {
    ...
    const tmpFileOut = `./tmp/${Random()}.webp`
    const tmpFileIn = `./tmp/${Random()}.webp`
